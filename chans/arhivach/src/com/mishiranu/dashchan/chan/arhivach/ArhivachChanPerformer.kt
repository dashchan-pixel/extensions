...
class ArhivachChanPerformer : ChanPerformer() {
			...
			for (tag in tags) {
				...
				try {
					...
					if (jsonArray == null || jsonArray.length() == 0) {
						if (tagsOnly) {
							throw HttpException(0, configuration.resources
	...
	private fun authorizeUserFromConfiguration(preset: HttpRequest.Preset): Pair<String, String>? {
		...
		val email = authorizationData?.get(0)
		val password = authorizationData?.get(1)
	...
	@Throws(HttpException::class, InvalidResponseException::class)
	private fun authorizeUser(preset: HttpRequest.Preset, email: String, password: String): Pair<String, String>? {
		userEmailPassword = null
		val locator = ChanLocator.get(this) as ArhivachChanLocator
		val uri = locator.buildPath("api", "add")
		try {
			val responseText = HttpRequest(uri, preset)
					.setPostMethod(UrlEncodedEntity("email", email, "pass", password)).perform().readString()
	...
	private fun updateAuthorizationData(jsonObject: JSONObject, emailPassword: Pair<String, String>?): Pair<String, String>? {
	...
	private class ArchiveRedirectHandler(private val locator: ArhivachChanLocator) : HttpRequest.RedirectHandler {
		var threadNumber: String? = null

		@Throws(HttpException::class)
		override fun onRedirect(response: HttpResponse): HttpRequest.RedirectHandler.Action {
			if (locator.isThreadUri(response.redirectedUri)) {
				threadNumber = locator.getThreadNumber(response.redirectedUri)
	...
	override fun onSendAddToArchive(data: SendAddToArchiveData): SendAddToArchiveResult {
		...
		while (true) {
			...
			if (userEmailPassword == null) {
				val captchaData = requireUserCaptcha(null, null, null, !first)
			...
			entity.add("add_collapsed", if (data.options.contains("collapsed")) "on" else null)
			...
			if (redirectHandler.threadNumber != null) {
				return SendAddToArchiveResult(null, redirectHandler.threadNumber)
			}
