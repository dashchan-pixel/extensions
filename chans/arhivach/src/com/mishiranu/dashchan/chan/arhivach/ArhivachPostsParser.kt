...
class ArhivachPostsParser(linked: Any, private val threadNumber: String) {
	...
	companion object {
		...
		val MONTHS_1: List<String> = listOf(
			"января", "февраля", "марта", "апреля", "мая", "июня",
			"июля", "августа", "сентября", "октября", "ноября", "декабря"
		)
		val MONTHS_2: List<String> = listOf(
			"Янв", "Фев", "Мар", "Апр", "Май", "Июн",
			"Июл", "Авг", "Сен", "Окт", "Ноя", "Дек"
		)

		@JvmStatic
		fun parseExpandImage(attributes: TemplateParser.Attributes, locator: ArhivachChanLocator): FileAttachment? {
			...
			.open { _, holder, _, attributes ->
				...
				if (StringUtils.isEmpty(number)) {
					if (holder.posts.size > 0) {
						number = holder.posts[holder.posts.size - 1].postNumber
						val index = number.indexOf('.')
						if (index >= 0) {
							number = number.substring(0, index) + "." + (number.substring(index + 1).toInt() + 1)
						} else {
							number += ".1"
						}
					} else {
