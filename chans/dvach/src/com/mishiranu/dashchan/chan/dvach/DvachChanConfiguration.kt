...
class DvachChanConfiguration : ChanConfiguration() {
	...
	override fun obtainBoardConfiguration(boardName: String?): Board = Board().apply {
		allowSearch = true
		allowCatalog = true
		allowArchive = true
		allowPosting = true
	...
	override fun obtainCustomCaptchaConfiguration(captchaType: String): Captcha? = when (captchaType) {
		CAPTCHA_TYPE_2CH_CAPTCHA -> Captcha().apply {
			title = "2ch Captcha"
			input = Captcha.Input.ALL
			validity = Captcha.Validity.IN_THREAD
			ttl = CAPTCHA_TTL
		}
		CAPTCHA_TYPE_2CH_EMOJI_CAPTCHA -> Captcha().apply {
			title = "Emoji Captcha"
			input = Captcha.Input.ALL
			validity = Captcha.Validity.IN_THREAD
			ttl = CAPTCHA_TTL
	...
	override fun obtainPostingConfiguration(boardName: String?, newThread: Boolean): Posting = Posting().apply {
		...
		attachmentCount = if (get(boardName, KEY_IMAGES_ENABLED, true)) {
	...
	override fun obtainReportingConfiguration(boardName: String?): Reporting = Reporting().apply {
		comment = true
		multiplePosts = true
	}

	override fun obtainVotingConfiguration(boardName: String?): Voting = Voting().apply {
		allowLike = true
		allowDislike = true
	}
	...
	fun updateFromBoardsJson(boardName: String, defaultName: String?, bumpLimit: Int?) {
	...
	fun updateFromThreadsPostsJson(boardName: String, configuration: DvachModelMapper.BoardConfiguration) {
	...
	private fun editBoards(boardName: String, key: String, value: Boolean?) {
		if (value != null) {
	...
	companion object {
		...
		@JvmField
		val CAPTCHA_TYPES: Map<String, String> = linkedMapOf(
