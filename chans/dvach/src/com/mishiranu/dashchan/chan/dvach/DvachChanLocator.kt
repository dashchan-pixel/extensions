...
class DvachChanLocator : ChanLocator() {
    ...
    fun createMobileApiV2Uri(vararg segments: String): Uri = buildPath("api", "mobile", "v2", *segments)

    enum class Fcgi(
        internal val fcgiName: String,
    ) {
        MAKABA("makaba"),
        POSTING("posting"),
    }
    ...
    ): Uri = buildQuery("makaba/${fcgi.fcgiName}.fcgi", *alternation)

    fun createCatalogSearchUri(
        boardName: String,
        query: String,
    ): Uri = buildQuery("$boardName/dashchan-query", "query", query)
    ...
    companion object {
        ...
        private val THREAD_PATH =
            Pattern.compile(
                "/\\w+/(?:arch/(?:\\d{4}-\\d{2}-\\d{2}/|wakaba/)?)?" +
                    "res/(\\d+)\\.html",
            )
        private val ATTACHMENT_PATH =
            Pattern.compile(
                "/\\w+/(?:arch/(?:\\d{4}-\\d{2}-\\d{2}/|wakaba/)?)?" +
                    "src/(\\d+)/\\d+\\.\\w+",
            )
