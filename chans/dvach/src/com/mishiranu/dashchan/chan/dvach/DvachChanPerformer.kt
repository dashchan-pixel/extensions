...
class DvachChanPerformer : ChanPerformer() {
    ...
    private fun buildCookiesWithCaptchaPass(): CookieBuilder = buildCookies(configuration.getCookie(COOKIE_PASSCODE_AUTH))
    ...
    override fun onReadThreads(data: ReadThreadsData): ReadThreadsResult {
        ...
        val response =
            HttpRequest(uri, data)
                .addCookie(buildCookiesWithCaptchaPass())
    ...
    @Throws(HttpException::class, InvalidResponseException::class, RedirectException::class)
    private fun onReadPosts(
        data: ReadPostsData,
        usePartialApi: Boolean,
        archive: Boolean,
    ): Posts {
        ...
        val uri: Uri =
            when {
                ...
                archive -> {
                    handler =
                        HttpRequest.RedirectHandler { response ->
    ...
    private fun parsePostsResponse(
                    ...
                    "posts" -> {
                        ...
                        if (posts.isNotEmpty()) {
                            ...
                            if (parentPostNumber != null && parentPostNumber != data.threadNumber) {
                                throw RedirectException.toThread(
                                    data.boardName,
                                    parentPostNumber,
                                    post.postNumber,
                                )
                ...
                when (reader.nextName()) {
                    "thread" -> {
                        ...
                        while (!reader.endStruct()) {
                            ...
                            posts.add(
                                DvachModelMapper.createWakabaArchivePost(
                                    reader,
                                    this,
                                    data.boardName,
                                ),
        ...
        try {
            for (i in 0 until jsonArray.length()) {
                ...
                if (!StringUtils.isEmpty(category) &&
                    ...
                    configuration.updateFromBoardsJson(
                        boardName,
                        defaultName,
    ...
    override fun onReadContent(data: ReadContentData): ReadContentResult =
        ReadContentResult(
            HttpRequest(data.uri, data.direct)
                .addCookie(buildCookiesWithCaptchaPass())
                .perform(),
        )
    ...
    @Throws(HttpException::class, InvalidResponseException::class)
    private fun onReadCaptcha(
        data: ReadCaptchaData,
        captchaPassData: String?,
        mayUseLastCaptchaPassCookie: Boolean,
    ): ReadCaptchaResult {
        val locator = this.locator
        val configuration = this.configuration
        ...
        val remoteCaptchaType =
            DvachChanConfiguration.CAPTCHA_TYPES[data.captchaType]
            ...
            if (id != null) {
                ...
                when {
                    DvachChanConfiguration.CAPTCHA_TYPE_2CH_CAPTCHA == data.captchaType -> {
                        ...
                        val imageUri =
                            locator
                                .buildPath("api", "captcha", remoteCaptchaType, "show")
                                .buildUpon()
                        ...
                        if (configuration.isFullKeyboardForCaptchaEnabled()) {
                            ...
                            result.setInput(
                                when (jsonObject.optString("input")) {
                    ...
                    DvachChanConfiguration.CAPTCHA_TYPE_2CH_EMOJI_CAPTCHA == data.captchaType -> {
                        ...
                        val retriever =
                            DvachEmojiCaptchaProvider.DvachEmojiCaptchaAnswerRetriever { task, keyboardImages ->
                                try {
                                    requireUserImageSingleChoice(
                                        -1,
                                        keyboardImages,
                                        configuration.resources.getString(R.string.emoji_captcha_input),
                                        task,
                                    )
                                } catch (e: HttpException) {
                                    -1
        ...
        if (!StringUtils.isEmpty(threadNumber)) {
            ...
            if (!StringUtils.isEmpty(originalPosterCookie)) {
                configuration.storeCookie(
                    originalPosterCookieName,
                    originalPosterCookie,
    ...
    override fun onSendReportPosts(data: SendReportPostsData): SendReportPostsResult? {
        ...
        try {
            ...
            var errorType = 0
            if (message?.contains("Вы уже отправляли жалобу") == true) {
                errorType = ApiException.REPORT_ERROR_TOO_OFTEN
            } else if (message?.contains("Вы ничего не написали в жалобе") == true) {
    ...
    override fun onSendVotePost(data: SendVotePostData): SendVotePostResult? {
        ...
        try {
            ...
            if (result != "1") {
                ...
                if (error?.contains("Постинг запрещён.") == true) {
    ...
    companion object {
        ...
        private val PREFERRED_BOARDS_ORDER =
            arrayOf(
                "Разное",
                "Тематика",
                "Творчество",
