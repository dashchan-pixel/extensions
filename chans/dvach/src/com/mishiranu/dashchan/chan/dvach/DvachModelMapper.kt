...
object DvachModelMapper {
            ...
            when (reader.nextName()) {
                ...
                "comment" -> {
                    ...
                    if (comment.contains("\"hashlink\"")) {
                        comment =
                            StringUtils.replaceAll(comment, PATTERN_HASHLINK) { matcher ->
    ...
    @JvmStatic
    fun createPostsFromHtml(html: String): List<String> = Jsoup.parse(html).select("div.box").map { it.id().split("post-")[1] }
