...
class FourchanChanConfiguration : ChanConfiguration() {
	...
	override fun obtainDeletingConfiguration(boardName: String?): Deleting = Deleting().apply {
		password = true
		multiplePosts = true
	...
	override fun obtainCaptchaPassConfiguration(): Authorization = Authorization().apply {
		fieldsCount = 2
	...
	fun isMathTagsHandlingEnabled(): Boolean {
		return get(null, KEY_MATH_TAGS, false)
	}

	fun isSafeForWork(boardName: String?): Boolean {
	...
	fun updateReportingConfiguration(boardName: String?, reportReasons: List<ReportReason>?) {
