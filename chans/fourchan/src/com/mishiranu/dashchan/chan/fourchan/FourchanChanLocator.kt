...
class FourchanChanLocator : ChanLocator() {
	...
	override fun isAttachmentUri(uri: Uri): Boolean =
			isChanHostOrRelative(uri) && (isPathMatches(uri, ATTACHMENT_PATH) || extractMathData(uri) != null)
	...
	override fun createBoardUri(boardName: String?, pageNumber: Int): Uri =
			if (pageNumber > 0) buildPathWithHost(getBoardsHost(boardName), boardName, "${pageNumber + 1}.html")
			else buildPathWithHost(getBoardsHost(boardName), boardName, "")

	override fun createThreadUri(boardName: String?, threadNumber: String): Uri =
			buildPathWithHost(getBoardsHost(boardName), boardName, "thread", threadNumber)
	...
	override fun createPostUri(boardName: String?, threadNumber: String, postNumber: String): Uri =
			createThreadUri(boardName, threadNumber).buildUpon().fragment("p$postNumber").build()
	...
	fun createBoardFlagIconUri(boardName: String, key: String): Uri {
		val fileName = key.lowercase(Locale.US) + ".gif"
	...
	fun buildMathUri(data: String): Uri {
		return try {
			buildPathWithHost(HOST_IMAGES, "math-tag",
	...
	override fun handleUriClickSpecial(uri: Uri): NavigationData? {
		if (isBoardUriOrSearch(uri)) {
			...
			if (query != null && query.startsWith("s=")) {
				return NavigationData(NavigationData.TARGET_SEARCH, getBoardName(uri),
