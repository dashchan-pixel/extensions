...
class FourchanChanPerformer : ChanPerformer() {
        ...
        if (FourchanChanConfiguration.CAPTCHA_TYPE_4CHAN_CAPTCHA == captchaType) {
            ...
            while (true) {
                try {
                    ...
                    if (jsonString.startsWith("<!DOCTYPE")) {
                    ...
                    if (e is HttpException) {
                        android.util.Log.e("FOURCHAN_DEBUG", "HttpException code: ${e.responseCode}")
                    }
            ...
            result =
                ReadCaptchaResult(CaptchaState.CAPTCHA, captchaData)
    ...
    override fun onSendPost(data: SendPostData): SendPostResult {
        ...
        if (captchaData != null) {
            ...
            captchaPassCookie = captchaData.get(CAPTCHA_DATA_KEY_PASS_COOKIE)
        }
    ...
    private inner class FourchanSpurResolver : FirewallResolver() {
        override fun checkResponse(
            ...
            if (responseText.contains("https://mcl.spur.us")) {
                ...
                if (matcher.find()) {
                    ...
                    val key = session.getKey(FirewallResolver.Identifier.Flag.USER_AGENT)
        ...
        private inner class Exclusive(
            ...
            override fun resolve(
                ...
                val client =
                    object : FirewallResolver.WebViewClient<String>("FourchanSpur") {
                ...
                if (!mclToken.isNullOrEmpty() && mclToken != "0") {
                    ...
                    val boardsHost = if (config.isSafeForWork(board)) "boards.4channel.org" else "boards.4chan.org"
                    val fourchanPassCookie = getFourchanPassCookie(config, board)
                    val entity = MultipartEntity()
                    entity.add("mcl", mclToken)
                    try {
                        val request =
                            HttpRequest(postUri, session)
                                .setPostMethod(entity)
                                .addHeader("Referer", "https://$boardsHost/")
                                .addHeader("Origin", "https://$boardsHost")
