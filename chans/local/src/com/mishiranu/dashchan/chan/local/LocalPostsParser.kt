...
class LocalPostsParser(linked: Any, private val threadNumber: String) : GroupParser.Callback {
	...
	fun convertThread(input: InputStream): Posts {
		...
		return Posts(post).addPostsCount(postsCount).addFilesCount(filesCount)
	}

	@Throws(OriginalPostParsedException::class)
	override fun onStartElement(parser: GroupParser, tagName: String,
			attributes: GroupParser.Attributes?): Boolean {
		if (attributes != null && attributes.contains("data-")) {
			...
			attributes.get("data-file")?.let {
				attachment = FileAttachment().also { attachment ->
	...
	private fun createFileUriLocal(uriString: String): Uri {
		...
		return if (uri.isRelative) {
			Uri.Builder().scheme("http").authority("localhost").path(uri.path).build()
		} else {
			uri
		}
	}
