...
open class VichanChanLocator : ChanLocator() {
    ...
    open fun createThreadsUri(boardName: String?, page: Int, isCatalog: Boolean): Uri? {
        return buildPath(
    ...
    open fun createBoardsUri(): Uri? {
        return buildPath(DEFAULT_SEGMENT_PRESET, "boards.json")
    }

    open fun createFileUri(boardName: String?, tim: String?, ext: String?): Uri? {
        return buildPath(DEFAULT_SEGMENT_PRESET, boardName, "src", "$tim$ext")
    }

    open fun createThumbnailUri(boardName: String?, thumbnail: String?): Uri? {
        return buildPath(DEFAULT_SEGMENT_PRESET, boardName, "thumb", thumbnail)
    }

    open fun createAntispamUri(boardName: String?, threadNumber: String?): Uri? {
