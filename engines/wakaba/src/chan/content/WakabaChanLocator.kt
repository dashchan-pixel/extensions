...
open class WakabaChanLocator : ChanLocator() {
	...
	open override fun isBoardUri(uri: Uri?): Boolean {
		return isChanHostOrRelative(uri) && isPathMatches(uri, boardPath)
	}

	open override fun isThreadUri(uri: Uri?): Boolean {
		return isChanHostOrRelative(uri) && isPathMatches(uri, threadPath)
	}

	open override fun isAttachmentUri(uri: Uri?): Boolean {
	...
	open override fun getThreadNumber(uri: Uri?): String? {
		return if (uri != null) getGroupValue(uri.path, threadPath, 1) else null
	}
	...
	open override fun createBoardUri(boardName: String?, pageNumber: Int): Uri {
		return if (pageNumber > 0) buildPath(boardName, "$pageNumber.html") else buildPath(boardName, "")
	}

	open override fun createThreadUri(boardName: String?, threadNumber: String?): Uri {
		return buildPath(boardName, "res", "$threadNumber.html")
	}

	open override fun createPostUri(boardName: String?, threadNumber: String?, postNumber: String?): Uri {
		return createThreadUri(boardName, threadNumber).buildUpon().fragment(postNumber).build()
	}

	open fun createScriptUri(boardName: String?, script: String?): Uri {
