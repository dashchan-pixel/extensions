...
abstract class WakabaChanPerformer : ChanPerformer() {
    ...
    protected abstract fun parseThreads(
        boardName: String?,
        input: InputStream,
    ): List<Posts>?
    ...
    protected abstract fun parsePosts(
        boardName: String?,
        input: InputStream,
    ): List<Post>?

    @Throws(HttpException::class, InvalidResponseException::class)
    protected open fun readPosts(
        data: ReadPostsData,
        uri: Uri,
        redirectHandler: HttpRequest.RedirectHandler?,
    ): List<Post> {
    ...
    @Throws(HttpException::class, InvalidResponseException::class)
    protected open fun readCaptchaResult(
    ...
    @Throws(HttpException::class, InvalidResponseException::class)
    protected open fun readCaptchaScript(
        data: ReadCaptchaData,
        script: String?,
    ): ReadCaptchaResult {
        val locator = ChanLocator.get(this) as WakabaChanLocator
        val uri =
            locator
                .createScriptUri(data.boardName, script)
                .buildUpon()
                .appendQueryParameter(
                    "key",
                    if (data.threadNumber == null) "mainpage" else "res" + data.threadNumber,
                ).build()
        return readCaptchaResult(data, uri)
    }

    @Throws(HttpException::class, InvalidResponseException::class)
    open override fun onReadCaptcha(data: ReadCaptchaData): ReadCaptchaResult = readCaptchaScript(data, "captcha.pl")
    ...
    companion object {
        ...
        private val POST_REDIRECT_HANDLER =
        ...
        @JvmStatic
        fun createSendPostEntity(
            data: SendPostData,
            mapper: SendPostEntityMapper?,
        ): RequestEntity {
