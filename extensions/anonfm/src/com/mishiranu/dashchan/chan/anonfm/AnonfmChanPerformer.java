...
public class AnonfmChanPerformer extends ChanPerformer
{
			...
			InvalidResponseException
	{
		...
		HttpResponse response = new HttpRequest(uri, preset).perform();
		...
		if (jsonArray != null && jsonArray.length() > 0)
		{
			...
			try
			{
				now = DATE_FORMAT_SERVER.parse(response.getHeaderFields().get("Date").get(0)).getTime();
			}
			...
			InvalidResponseException
	{
		if (AnonfmChanLocator.BOARD_NAME_FM.equals(data.boardName))
		{
			...
			try
			{
				...
				calendar.setTime(DATE_FORMAT_SERVER.parse(response.getHeaderFields().get("Last-Modified").get(0)));
