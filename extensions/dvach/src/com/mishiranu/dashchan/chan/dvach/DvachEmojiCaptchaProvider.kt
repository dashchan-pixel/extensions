...
internal class DvachEmojiCaptchaProvider(
	...
	fun loadEmojiCaptcha(): ChanPerformer.ReadCaptchaResult {
		val uri = locator.buildPath("api", "captcha", "emoji", "show").buildUpon()
	...
	private fun solveEmojiCaptchaLoop(
		...
		when (parsedResponse) {
			is EmojiCaptchaResponse.Content -> {
				...
				val keyboardImages = Array(parsedResponse.keyboard.size) { i ->
	...
	@Throws(HttpException::class)
	private fun doWithRetries(uri: Uri, data: HttpRequest.Preset, attempts: Int): HttpResponse {
	...
	private fun createTaskWithSelectedBitmap(captchaImage: Bitmap, selected: SelectedEmojis): Bitmap {
	...
	internal fun interface DvachEmojiCaptchaAnswerRetriever {
		 ...
		 */
		fun getAnswer(task: Bitmap, keyboard: Array<Bitmap>): Int?
	}
	...
	private sealed class EmojiCaptchaResponse {
		...
		class Success(val success: String) : EmojiCaptchaResponse()

		/** Received while captcha solving is in progress. */
		class Content(val image: String, val keyboard: List<String>) : EmojiCaptchaResponse()
	}
