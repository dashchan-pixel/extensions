...
public final class WebSocket {
	...
	public Connection open(EventHandler handler) throws HttpException {
		...
		try {
			...
			boolean verifyCertificate = locator.isUseHttps();
	...
	private SocketResult openSocket(Uri uri, String chanName, boolean verifyCertificate, int attempts)
		...
		try {
			...
			if ("https".equals(scheme) || "wss".equals(scheme)) {
				...
				throw new HttpException(0, "Error");
			}
			...
			if (secure) {
				SSLSocket sslSocket = (SSLSocket) javax.net.ssl.HttpsURLConnection.getDefaultSSLSocketFactory()
			...
			CookieBuilder cookieBuilder = this.cookieBuilder;
			if (cookieBuilder != null) {
				requestBuilder.append("Cookie: ").append(cookieBuilder.build().replaceAll("[\r\n]", ""))
						.append("\r\n");
			}
			if (addUserAgent) {
				requestBuilder.append("User-Agent: ").append(System.getProperty("http.agent")
						.replaceAll("[\r\n]", "")).append("\r\n");
			}
			requestBuilder.append("\r\n");
			...
			while (true) {
				...
				if (b == -1) {
					throw new HttpException(0, "Timeout");
				}
	...
	private void checkException() throws HttpException {
		...
		try {
			...
			if (exception != null) {
				...
				throw new HttpException(0, exception.getMessage());
			}
	...
	private static Uri obtainRedirectedUri(Uri baseUri, String location) {
		if (location == null) return null;
		Uri uri = Uri.parse(location);
		if (uri.isAbsolute()) return uri;
		return baseUri.buildUpon().encodedPath(location).build();
	}
	...
	private static class IOUtils {
		...
		public static boolean readExactlyCheck(InputStream in, byte[] buffer, int offset, int length) throws IOException {
			int read = 0;
			while (read < length) {
		...
		public static int bytesToInt(boolean bigEndian, int offset, int length, byte[] bytes) {
			int value = 0;
			for (int i = 0; i < length; i++) {
				int shift = (bigEndian ? length - 1 - i : i) * 8;
				value |= (bytes[offset + i] & 0x000000FF) << shift;
			}
			return value;
		}
		public static byte[] intToBytes(int value, boolean bigEndian, int offset, int length, byte[] bytes) {
			if (bytes == null) bytes = new byte[length];
			for (int i = 0; i < length; i++) {
				int shift = (bigEndian ? length - 1 - i : i) * 8;
				bytes[offset + i] = (byte) ((value >> shift) & 0xFF);
			}
			return bytes;
		}
	}
