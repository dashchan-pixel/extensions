...
public class PonychanPostsParser implements GroupParser.Callback
{
	...
	public boolean onStartElement(GroupParser parser, String tagName, String attrs) throws ParseException
	{
		...
		{
			String id = GroupParser.extractAttr(attrs, "id");
			if (id != null && id.startsWith("reply_"))
			{
				String number = id.substring(6, id.length());
				boolean originalPost = GroupParser.extractAttr(attrs, "class").contains("op ");
				Post post = new Post();
				if (originalPost)
				{
			...
			{
				String cssClass = GroupParser.extractAttr(attrs, "class");
		...
		else if ("span".equals(tagName))
		{
			if (mPost != null)
			{
				...
				{
					String originalName = GroupParser.extractAttr(attrs, "title");
		...
		{
			String datetime = GroupParser.extractAttr(attrs, "datetime");
			if (datetime != null)
			{
				try
				{
		...
		else if ("img".equals(tagName))
		{
			...
			{
				String src = GroupParser.extractAttr(attrs, "src");
				if ("/static/spoiler.png".equals(src))
				{
					src = GroupParser.extractAttr(attrs, "data-mature-src");
			...
			{
				String alt = GroupParser.extractAttr(attrs, "alt");
		...
		else if ("a".equals(tagName))
		{
			...
			{
				Uri uri = Uri.parse(GroupParser.extractAttr(attrs, "href"));
			...
			else
			{
				...
				{
					mPost.setEmail(GroupParser.extractAttr(attrs, "href"));
				}
			}
		}
		...
		else if ("form".equals(tagName))
		{
			...
			{
				String name = GroupParser.extractAttr(attrs, "name");
