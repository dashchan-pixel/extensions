...
public class TinyboardAntispamFieldsParser implements GroupParser.Callback
{
	 ...
	 */
	private static final List<String> VALID_FIELDS = Arrays.asList(new String[] {"board", "thread", "making_a_post",
			"name", "email", "subject", "post", "body", "file", "activate_egg", "password",
			"mature", "spoiler", "spoiler_thread"});
	...
	public boolean onStartElement(GroupParser parser, String tagName, String attrs)
	{
		...
		else if (mFormParsing)
		{
			...
			if (input || textarea)
			{
				String fieldName = StringUtils.clearHtml(GroupParser.extractAttr(attrs, "name"));
				if (!VALID_FIELDS.contains(fieldName))
				{
					...
					{
						String value = StringUtils.unescapeHtml(GroupParser.extractAttr(attrs, "value"));
