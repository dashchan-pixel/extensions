...
public class PrincessLunaChanPerformer extends ChanPerformer
{
	@Override
	public ReadThreadsResult onReadThreads(ReadThreadsData data) throws HttpException, InvalidResponseException
	{
		PrincessLunaChanLocator locator = ChanLocator.get(this);
		...
		if (jsonObject != null && data.pageNumber >= 0)
		{
			try
			{
				...
				{
					threads[i] = PrincessLunaModelMapper.createThread(threadsArray.getJSONObject(i),
			...
			if (data.isCatalog())
			{
				try
				{
					...
					for (int i = 0; i < jsonArray.length(); i++)
					{
						...
						{
							threads.add(PrincessLunaModelMapper.createThread(threadsArray.getJSONObject(j),
		...
		if (jsonObject != null)
		{
			try
			{
				...
				if (jsonArray.length() > 0)
				{
					...
					{
						posts[i] = PrincessLunaModelMapper.createPost(jsonArray.getJSONObject(i),
	...
	public ReadBoardsResult onReadBoards(ReadBoardsData data) throws HttpException, InvalidResponseException
	{
		...
		try
		{
			return new ReadBoardsResult(new PrincessLunaBoardsParser(responseText).convert());
		}
	...
	private JSONObject getJsonObject(HttpResponse response) throws HttpException, InvalidResponseException
	{
	...
	public SendPostResult onSendPost(SendPostData data) throws HttpException, ApiException, InvalidResponseException
	{
		...
		if (errorMessage != null)
		{
			...
			CommonUtils.writeLog("PrincessLuna send message", errorMessage);
			...
			InvalidResponseException
	{
		...
		if (errorMessage != null)
		{
			...
			if (errorType != 0) throw new ApiException(errorType);
			CommonUtils.writeLog("PrincessLuna delete message", errorMessage);
			throw new ApiException(errorMessage);
		}
		throw new InvalidResponseException();
	}
			...
			InvalidResponseException
	{
		...
		if (errorMessage != null)
		{
			CommonUtils.writeLog("PrincessLuna report message", errorMessage);
			throw new ApiException(errorMessage);
		}
		throw new InvalidResponseException();
	}
