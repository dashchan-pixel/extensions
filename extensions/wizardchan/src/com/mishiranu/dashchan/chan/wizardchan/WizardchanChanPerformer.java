...
public class WizardchanChanPerformer extends ChanPerformer {
	private void checkResponse(HttpResponse response) throws HttpException {
		if (response != null && !Collections.singletonList("application/json").equals(response.getHeaderFields().get("Content-Type"))) {
	...
	public ReadPostsResult onReadPosts(ReadPostsData data) throws HttpException, InvalidResponseException {
		...
		HttpResponse response = new HttpRequest(uri, data.holder, data).setValidator(data.validator).perform();
		JSONObject jsonObject = response.getJsonObject();
		if (jsonObject != null) {
			try {
