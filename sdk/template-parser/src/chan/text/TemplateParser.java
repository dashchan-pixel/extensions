...
public final class TemplateParser<H> {
	...
	private final ArrayList<NewTextCallback<H>> newTextCallbacks = new ArrayList<>();
	...
	public TemplateParser<H> text(NewTextCallback<H> textCallback) {
		checkReady();
		copyCallbacks();
		if (!buildingMatchers.isEmpty()) {
			throw new IllegalStateException("This callback can not be used with any parsing rules");
	 ...
	 */
	public interface NewTextCallback<H> {
	...
	private static class Implementation<H> implements GroupParser.Callback {
		...
		public void onText(GroupParser parser, CharSequence text) throws ParseException {
			...
			ArrayList<NewTextCallback<H>> newTextCallbacks = this.parser.newTextCallbacks;
			for (NewTextCallback<H> newTextCallback : newTextCallbacks) {
	...
	public interface InitialBuilder<H> extends SimpleRuleBuilder<H>, ComplexRuleBuilder<H> {
		...
		InitialBuilder<H> text(NewTextCallback<H> textCallback);
	...
	private final ContentBuilder<H> contentBuilder = new ContentBuilder<H>() {
		...
		@Override
		public InitialBuilder<H> text(NewTextCallback<H> textCallback) {
			TemplateParser.this.text(textCallback);
			return contentBuilder;
		}
